...
def _admin_password() -> str:
    if password := os.getenv("opanel_ent_ADMIN_PASSWORD"):
        if len(password) < 12:
            raise ValueError("opanel_ent_ADMIN_PASSWORD must be at least 12 characters")
        if any(char in password for char in (":", "\r", "\n", "\x00")):
            raise ValueError("opanel_ent_ADMIN_PASSWORD cannot contain ':', newlines, or NUL characters")
        return password
