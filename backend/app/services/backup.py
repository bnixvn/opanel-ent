...
logger = logging.getLogger("opanel-ent.backup")
...
# Full-user archives opanel-ent will restore. opanel-ent writes "opanel_ent_user"; "bpanel_user"
# is the same archive layout (sites/<domain>/site, databases/<domain>.sql, a
# version-1 website schema) written by bpanel, opanel-ent's predecessor, so accept it
...
RESTORABLE_BACKUP_KINDS = ("opanel_ent_user", "bpanel_user")
_PLACEHOLDER_EMAIL_SUFFIXES = (
    "@users.opanel-ent.test", "@users.opanel-ent.vn",
...
def create_user_backup(user: User, db) -> str:
    ...
    with tempfile.TemporaryDirectory(prefix="opanel-ent-user-backup-", dir=str(backup_dir)) as tmp:
        tmp_dir = Path(tmp)
        manifest = {
            "kind": "opanel_ent_user",
            "version": 1,
...
def restore_user_backup(backup_file: str, db) -> dict:
    ...
    created_user = False
    if user is None:
        email = user_info.get("email") or f"{username}@users.opanel-ent.invalid"
        if email.endswith(_PLACEHOLDER_EMAIL_SUFFIXES):
            email = f"{username}@users.opanel-ent.invalid"
    ...
    restored_websites = []
    with tempfile.TemporaryDirectory(prefix="opanel-ent-user-restore-") as tmp:
        tmp_dir = Path(tmp)
        for site_info in manifest.get("websites") or []:
            ...
            # Backups from older opanel-ent releases may contain public/. Normalize
...
def restore_backup_database(website: Website, backup_file: str, db_name: str) -> bool:
    ...
    """
    archive = backup_path(website.domain, backup_file)
    with tempfile.TemporaryDirectory(prefix="opanel-ent-db-restore-") as tmp_dir:
        sql_path = None
        with tarfile.open(archive, "r:gz") as tar:
...
def upload_to_sftp(
    ...
    *,
    ...
    # Note: we deliberately do NOT call load_system_host_keys() because the
        # daemon runs as the opanel-ent service user with no interactive shell
